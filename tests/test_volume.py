...
def test(change_params, device, config_file):
...
def run_tests_on(config_file, devices=['cpu', 'cuda']):
    for device in devices:
        test(grayscale_color_space, device, config_file)
        test(rgb_color_space, device, config_file)
        test(ycbcr_color_space, device, config_file)
        test(lnet_architecture, device, config_file)
        test(mnet_architecture, device, config_file)
        test(not_periodic, device, config_file)
        
        print('*' * 14, f"\nAll tests on {device} succeed!\n", '*' * 14, '\n')

if __name__ == '__main__':
    # run_tests_on('configs/tests/image.yml')
    run_tests_on('configs/tests/volume.yml')
